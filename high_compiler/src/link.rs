...
pub fn link(ast: &ast::Program) -> anyhow::Result<Vec<Arc<Proc>>> {
    ...
    fn gen_body(items: impl IntoIterator<Item = Arc<ast::BodyItem>>) -> Arc<ast::Body> {
        Arc::new(ast::Body { items: Arc::new(items.into_iter().collect()) })
    }
    ...
    let out_func = gen_func_item(ast::Func {
        name: gen_name("out"),
        params: Arc::new(Vec::from([gen_decl("val", ast::Type::Base(gen_base_ty("any")))])),
    ...
    let in_func = gen_func_item(ast::Func {
        ...
        proc: Arc::new(ast::Proc {
            idents: Arc::new(Vec::from([gen_decl("answer", ast::Type::Base(gen_base_ty("val")))])),
            body: gen_body([
                ...
                gen_statement_item(ast::Statement::Assign(Arc::new(ast::Assign {
                    place: gen_deref(Arc::new(ast::Expr::Place(gen_ident("dest_ref")))),
    ...
    let random_func = gen_func_item(ast::Func {
        name: gen_name("random"),
        params: Arc::new(Vec::from([
            gen_decl("dest_ref", ast::Type::Ref(gen_ref_ty(ast::Type::Base(gen_base_ty("val"))))),
            gen_decl("min", ast::Type::Base(gen_base_ty("val"))),
            gen_decl("max", ast::Type::Base(gen_base_ty("val"))),
        ])),
        proc: Arc::new(ast::Proc {
            idents: Arc::new(Vec::from([gen_decl(
                "generated",
                ast::Type::Base(gen_base_ty("val")),
            )])),
            body: gen_body([
                gen_statement_item(ast::Statement::Native(Arc::new(
                    ast::NativeOperation::Random {
                        dest_ident: gen_ident("generated"),
                        min: Arc::new(ast::Expr::Place(gen_ident("min"))),
                        max: Arc::new(ast::Expr::Place(gen_ident("max"))),
                    },
                ))),
                gen_statement_item(ast::Statement::Assign(Arc::new(ast::Assign {
                    place: gen_deref(Arc::new(ast::Expr::Place(gen_ident("dest_ref")))),
    ...
    let top_items =
        chain!(ast.items.iter().cloned(), [out_func, in_func, random_func]).collect_vec();
