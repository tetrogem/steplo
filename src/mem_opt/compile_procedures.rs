...
pub fn compile<'a>(
    ...
    let mut compile_m = CompileManager {
        ...
        dyn_custom_block: Arc::new(ez::CustomBlock { uuid: Uuid::new_v4(), name: "dyn %s".into() }),
    ...
    let dyn_stacks = {
        let proc_arg_op = Arc::new(ez::Op::Argument(ez::ArgumentOp::ReporterStringNumber {
            arg: compile_m.dyn_proc_argument.clone(),
        }));
        ...
        for custom_block in compile_m.sp_uuid_to_custom_block.values() {
                ...
                None => Arc::new(ez::Op::Control(ez::ControlOp::If {
                    condition: Arc::new(ez::Expr::Derived(Arc::new(ez::Op::Operator(
                        ez::OperatorOp::Equals {
                            operand_a: Arc::new(ez::Expr::Derived(proc_arg_op.clone())),
        ...
        let prototype = Arc::new(ez::Expr::Stack(Arc::new(ez::Stack {
            root: Arc::new(ez::Op::Procedure(ez::ProcedureOp::Prototype {
                custom_block: compile_m.dyn_custom_block.clone(),
                arguments_with_stacks: Arc::new(Vec::from([(
                    compile_m.dyn_proc_argument.clone(),
                    proc_arg_stack,
                )])),
            })),
            rest: Arc::new(Vec::new()),
        ...
        Vec::from([ez::Stack { root: def, rest: Arc::new(branch_stack.into_iter().collect()) }])
    };

    let stacks = proc_stacks.into_iter().chain(dyn_stacks).map(Arc::new).collect();
...
fn compile_proc(compile_m: &mut CompileManager, proc: &Proc<RMemLoc>) -> Vec<ez::Stack> {
    ...
    for (i, sp) in proc.sub_procs.iter().enumerate() {
        ...
        let root = match is_program_start {
            ...
            false => ez::Op::Procedure(ez::ProcedureOp::Definition {
                prototype_stack: Arc::new(ez::Expr::Stack(Arc::new(ez::Stack {
                    root: Arc::new(ez::Op::Procedure(ez::ProcedureOp::Prototype {
                        custom_block: compile_m.get_custom_block(&sp.uuid).clone(),
...
fn compile_call(compile_m: &mut CompileManager, call: &Call<RMemLoc>) -> Vec<ez::Op> {
    match call {
        Call::Exit => Vec::new(),
        Call::Jump(to) => Vec::from([ez::Op::Procedure(ez::ProcedureOp::Call {
            custom_block: compile_m.dyn_custom_block.clone(),
            argument_inputs: Arc::new(Vec::from([(
                compile_m.dyn_proc_argument.clone(),
                compile_expr(compile_m, to),
            )])),
        })]),
            ...
            Vec::from([ez::Op::Control(ez::ControlOp::IfElse {
                ...
                then_substack: Arc::new(ez::Expr::Stack(Arc::new(ez::Stack {
                    root: Arc::new(ez::Op::Procedure(ez::ProcedureOp::Call {
                        ...
                        argument_inputs: Arc::new(Vec::from([(
                            compile_m.dyn_proc_argument.clone(),
                            compile_expr(compile_m, then_to),
                        )])),
                    })),
                    rest: Arc::new(Vec::new()),
                }))),
                else_substack: Arc::new(ez::Expr::Stack(Arc::new(ez::Stack {
                    root: Arc::new(ez::Op::Procedure(ez::ProcedureOp::Call {
                        ...
                        argument_inputs: Arc::new(Vec::from([(
                            compile_m.dyn_proc_argument.clone(),
                            compile_expr(compile_m, else_to),
                        )])),
                    })),
                    rest: Arc::new(Vec::new()),
                }))),
...
fn compile_expr(compile_m: &mut CompileManager, expr: &Expr<RMemLoc>) -> Arc<ez::Expr> {
    let compiled_expr = match expr {
        ...
        Expr::Value(value) => match value.as_ref() {
            ...
            Value::Label(label) => ez::Expr::Literal(Arc::new(ez::Literal::String(
                compile_m.get_custom_block(label).name.clone(),
            ))),
        },
